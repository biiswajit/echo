...
export class ConversationMetadataCache extends Cache<ConversationMetadataType> {
	...
	static async getInstance(): Promise<ConversationMetadataCache> {
	...
	async read(key: CacheKeyType): Promise<CacheReturnType<ConversationMetadataType>> {
		try {
			if (!this.client || !key) {
	...
	async write(key: CacheKeyType, payload: ConversationMetadataType): Promise<boolean> {
		try {
			if (!this.client || !key) {
				throw new Error("no client or conversation id found");
			}
